...
// scopeCountingResolver returns an unscoped TAT for unscoped specs and a scoped TAT
// for scope-bound specs (spec.Scopes != ""), counting total resolves and scoped
// (re-)mints so the missing-scope retry tests can assert exactly one mint and that
// a later call reuses the cached scoped token (re-resolves nothing).
type scopeCountingResolver struct {
	unscoped, scoped string
	calls            int // total ResolveToken calls
	scopedMints      int // scope-bound mints only
}

func (r *scopeCountingResolver) ResolveToken(_ context.Context, spec credential.TokenSpec) (*credential.TokenResult, error) {
	r.calls++
	if spec.Scopes != "" {
		r.scopedMints++
		return &credential.TokenResult{Token: r.scoped, Scopes: spec.Scopes}, nil
	}
	return &credential.TokenResult{Token: r.unscoped}, nil
}

// newScopeRetryClient builds an APIClient whose SDK talks through rt and whose
// Credential resolves tokens via resolver (App ID "test-app").
func newScopeRetryClient(rt http.RoundTripper, resolver credential.DefaultTokenResolver) *APIClient {
	sdk := lark.NewClient("test-app", "test-secret",
		lark.WithEnableTokenCache(false),
		lark.WithLogLevel(larkcore.LogLevelError),
...
// respBodyCode extracts the business code from an ApiResp body.
func respBodyCode(t *testing.T, resp *larkcore.ApiResp) int {
	t.Helper()
	var body struct {
		Code int `json:"code"`
	}
	if err := json.Unmarshal(resp.RawBody, &body); err != nil {
		t.Fatalf("parse resp body %q: %v", string(resp.RawBody), err)
	}
	return body.Code
}

// grantedScopesResponse is the application-info payload appGrantedScopes parses;
// only scopes whose token_types include "user" are collected (here: im:message).
func grantedScopesResponse() *http.Response {
	return jsonResponse(map[string]any{
		"code": 0,
		"data": map[string]any{"app": map[string]any{"scopes": []map[string]any{
			{"scope": "im:message", "token_types": []string{"user", "tenant"}},
			{"scope": "contact:contact", "token_types": []string{"tenant"}}, // tenant-only → excluded
		}}},
	})
}

func scopeRetryReq() *larkcore.ApiReq {
	return &larkcore.ApiReq{HttpMethod: http.MethodGet, ApiPath: "/open-apis/im/v1/messages", QueryParams: larkcore.QueryParams{}}
}

// TestDoSDKRequest_MissingScope_RetriesAndReusesScopedToken exercises the full
// DoSDKRequest auto-retry: an initial bot call hits 99991679, DoSDKRequest mints a
// full-scope bot TAT and retries successfully, and a subsequent call reuses the
// cached scoped token — the scoped mint happens exactly once and the second call
// re-resolves nothing.
func TestDoSDKRequest_MissingScope_RetriesAndReusesScopedToken(t *testing.T) {
	const unscoped, scoped = "t-unscoped", "t-scoped"
	resolver := &scopeCountingResolver{unscoped: unscoped, scoped: scoped}
	appInfoPath := internalauth.ApplicationInfoPath("test-app")

	var bizCalls int
	var lastBizAuth string
	rt := roundTripFunc(func(req *http.Request) (*http.Response, error) {
		if req.URL.Path == appInfoPath {
			return grantedScopesResponse(), nil
		}
		bizCalls++
		lastBizAuth = req.Header.Get("Authorization")
		if bizCalls == 1 { // initial call carries the unscoped token → missing scope
			return jsonResponse(map[string]any{"code": output.LarkErrUserScopeInsufficient, "msg": "missing scope"}), nil
		}
		return jsonResponse(map[string]any{"code": 0, "data": map[string]any{}}), nil // retry + reuse succeed
	})

	ac := newScopeRetryClient(rt, resolver)

	// Call A: 99991679 → mint scoped → retry ok.
	respA, err := ac.DoSDKRequest(context.Background(), scopeRetryReq(), core.AsBot)
	if err != nil {
		t.Fatalf("call A: unexpected error: %v", err)
	}
	if code := respBodyCode(t, respA); code != 0 {
		t.Fatalf("call A: code = %d, want 0 (retry must succeed)", code)
	}
	if resolver.scopedMints != 1 {
		t.Fatalf("after call A: scoped mints = %d, want 1", resolver.scopedMints)
	}
	callsAfterA := resolver.calls

	// Call B: must reuse the cached scoped token — no new mint, no re-resolve.
	respB, err := ac.DoSDKRequest(context.Background(), scopeRetryReq(), core.AsBot)
	if err != nil {
		t.Fatalf("call B: unexpected error: %v", err)
	}
	if code := respBodyCode(t, respB); code != 0 {
		t.Fatalf("call B: code = %d, want 0", code)
	}
	if resolver.scopedMints != 1 {
		t.Errorf("scoped mints = %d, want 1 (token cached, not re-minted)", resolver.scopedMints)
	}
	if resolver.calls != callsAfterA {
		t.Errorf("call B re-resolved a token (calls %d→%d); it must reuse the cached scoped token",
			callsAfterA, resolver.calls)
	}
	if bizCalls != 3 { // A initial + A retry + B
		t.Errorf("business calls = %d, want 3", bizCalls)
	}
	if !strings.Contains(lastBizAuth, scoped) {
		t.Errorf("call B Authorization = %q, want it to carry the scoped token %q", lastBizAuth, scoped)
	}
}

// TestDoSDKRequest_MissingScope_RetryAlsoFailsDoesNotLoop pins that the retry runs
// at most once: when the scoped retry ALSO returns 99991679, DoSDKRequest surfaces
// that response instead of looping — exactly one scoped mint, two business calls.
func TestDoSDKRequest_MissingScope_RetryAlsoFailsDoesNotLoop(t *testing.T) {
	const unscoped, scoped = "t-unscoped", "t-scoped"
	resolver := &scopeCountingResolver{unscoped: unscoped, scoped: scoped}
	appInfoPath := internalauth.ApplicationInfoPath("test-app")
	...
	rt := roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		bizCalls++ // every business call (incl. the scoped retry) misses scope
		return jsonResponse(map[string]any{"code": output.LarkErrUserScopeInsufficient, "msg": "missing scope"}), nil
	})

	ac := newScopeRetryClient(rt, resolver)
	resp, err := ac.DoSDKRequest(context.Background(), scopeRetryReq(), core.AsBot)
	if err != nil {
		t.Fatalf("unexpected transport error: %v", err)
	}
	if bizCalls != 2 {
		t.Errorf("business calls = %d, want 2 (one retry, no loop)", bizCalls)
	}
	if resolver.scopedMints != 1 {
		t.Errorf("scoped mints = %d, want 1", resolver.scopedMints)
	}
	if code := respBodyCode(t, resp); code != output.LarkErrUserScopeInsufficient {
		t.Errorf("resp code = %d, want %d (missing-scope surfaced, not swallowed)", code, output.LarkErrUserScopeInsufficient)
	}
}
